    ...
    if current_user.is_authenticated:
        ...
        for destination in sendMessageTo:
            ...
            if UserCheck is not None:
                existingBanSearch = banList.messageBanList.query.filter_by(
                    userID=UserCheck.id, messageFrom=current_user.id
                ).first()
                if existingBanSearch is None:
                    message_tasks.send_message.delay(
                        messageSubject, messageContent, current_user.id, UserCheck.id
                    )
                else:
                    emit(
                        "messageBanWarning",
                        {
                            "message": "User "
                            + UserCheck.username
                            + " has blocked messages from your account. Message Not Sent!"
                        },
                        broadcast=False,
                    )
    return "OK"
...
def getMessage(message):
    if current_user.is_authenticated:
        ...
        messageQuery = notifications.userMessage.query.filter_by(
            id=messageID, toUserID=current_user.id
        ).first()
        if messageQuery != None:
            ...
            emit(
                ...
                {
                    "status": "success",
                    "fromUser": messageQuery.fromUserID,
                    "fromUsername": fromUserQuery.username,
                    "fromUserPhoto": fromUserQuery.pictureLocation,
                    "subject": messageQuery.subject,
                    "timestamp": str(messageQuery.timestamp),
                    "content": md.markdown(messageQuery.message),
                    "id": messageQuery.id,
                },
                broadcast=False,
            )
    ...
    if current_user.is_authenticated:
        if "messageId" in message:
            ...
            for message in messages:
                messageQuery = notifications.userMessage.query.filter_by(
                    id=int(message), toUserID=current_user.id
                ).first()
        ...
        if "banListUsers" in message:
            ...
            for user in requestedBanList:
                ...
                if UserCheck is not None and current_user.id != int(user["value"]):
                    existingBanSearch = banList.messageBanList.query.filter_by(
                        userID=current_user.id, messageFrom=UserCheck.id
                    ).first()
                    if existingBanSearch is None:
...
def removeFromBanList(message):
    if current_user.is_authenticated:
        if "userID" in message:
            ...
            existingBanSearch = banList.messageBanList.query.filter_by(
                userID=current_user.id, messageFrom=int(userID)
            ).first()
